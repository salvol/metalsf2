...
from spekpy.SpekHelpers import get_current_time_stamp, format_parameter_str as fmt_param, ord_dct
from spekpy.IO import write_json_to_disk, read_json_from_disk, write_spectrum_to_disk, is_file, full_file
...
class State(object):
    ...
    def export_spectrum_to_disk(self, file_name, delimiter):
        ...
        """
        if not file_name:
            file_name = 'spekpy_export ' + get_current_time_stamp(mode='file') + Const.extension_export_spectrum_file

        header = 'Header Begin\n'
        header = header + self.get_basic_info_str(file_type='exported_spectrum') + '\n'
        header = header + self.get_current_state_str(mode='full')
        header = header + 'Spectrum Units:\n[keV], [Photons keV^-1 cm^-2], [Photons keV^-1 cm^-2]' + '\n'
    ...
    def get_basic_info_str(self, file_type='-'):
        ...
        :return str info_str: String containing spekpy version, filetype and comment  
        """
    ...
    def get_current_results_str(self):
        ...
        result_str = fmt_param('Fluence', res['flu'], 'Photons cm^-2', decimal_places=4) + ' ' + \
                     fmt_param('Air Kerma', res['kerma'], 'uGy', decimal_places=4) + \
                     fmt_param('Mean Energy', res['emean'], 'keV', decimal_places=4) + '\n'
        result_str = result_str + fmt_param('First HVL Al', res['hvl_1_al'], 'mm Al', decimal_places=4) + ' ' + \
                     fmt_param('Second HVL Al', res['hvl_2_al'], 'mm Al', decimal_places=4) + ' ' + \
                     fmt_param('Homogeneity Coefficient Al', res['hc_al'], '-', decimal_places=4) + \
                     fmt_param('Effective Energy Al', res['eeff_al'], 'keV', decimal_places=4) + '\n'
        result_str = result_str + fmt_param('First HVL Cu', res['hvl_1_cu'], 'mm Cu', decimal_places=4) + ' ' + \
                     fmt_param('Second HVL Cu', res['hvl_2_cu'], 'mm Cu', decimal_places=4) + ' ' + \
                     fmt_param('Homogeneity Coefficient Cu', res['hc_cu'], '-', decimal_places=4) + \
                     fmt_param('Effective Energy Cu', res['eeff_cu'], 'keV', decimal_places=4)
        return result_str

    def get_spectrum_str(self):
        """
        A method to format the spectrum energy-fluence values into a single string
    ...
    def get_current_state_str(self, mode):
        """
        A method to summarize the current state of the spekpy instance as a single string

        :return str current_state_str: A string with spekpy's current state
        """

        current_state_str = 'Inputs\n' + '-' * 6 + '\n'
        current_state_str = current_state_str + self.get_model_parameters_str() + '\n'
        current_state_str = current_state_str + self.get_spectrum_parameters_str() + '\n'
        current_state_str = current_state_str + self.get_filtration_str() + '\n'

        if mode == 'minimal':
            pass
        elif mode == 'full':
            current_state_str = current_state_str + 'Outputs\n' + '-' * 7 + '\n'
            current_state_str = current_state_str + self.get_current_results_str() + '\n'

        return current_state_str

    def prepare_save_state(self, file_name, file_type='state_usr', extra_param_name=None, extra_param=None):
        """
        A method to summarize the current state as an ordered dictionary, ready for saving state
        ...
        prepare_state = [('file_information', self.file_info_dct(file_name, file_type)),
                         ('external_spectrum', self.external_spectrum_dct()),
                         ('model_parameters', self.model_parameters_dct()),
                         ('spectrum_parameters', self.spectrum_parameters_dct()),
                         ('filters', self.filtration.filters),
                         ('flags', self.flags_dct())]
    ...
    def file_info_dct(self, file_name, file_type):
        """
        A method to summarize basic information for a file as an ordered dictionary

        :param str file_name: Name of a file
        :param str file_type: The type of file
        :return dict file_information: An ordered dictionary containing basic information on file
        """
    ...
    def external_spectrum_dct(self):
        ...
        :return dict external_spectrum: An ordered dictionary containing information about an external spectrum.
        """
        external_spectrum = ord_dct([('external_spectrum', self.external_spectrum.external_spectrum),
                                     ('ext_delim', self.external_spectrum.ext_delim),
                                     ('ext_z', self.external_spectrum.ext_z),
                                     ('ext_mas', self.external_spectrum.ext_mas)])
    ...
    def model_parameters_dct(self):
        ...
        :return dict model_parameters: An ordered dictionary containing information about spectrum model
        """
        model_parameters = ord_dct([('kvp', self.model_parameters.kvp),
                                    ('th', self.model_parameters.th),
                                    ('dk', self.model_parameters.dk),
                                    ('physics', self.model_parameters.physics),
                                    ('mu_data_source', self.model_parameters.mu_data_source)])
    ...
    def spectrum_parameters_dct(self):
        ...
        :return dict spectrum_parameters: An ordered dictionary containing information about spectrum parameters
        """
        spectrum_parameters = ord_dct([('x', self.spectrum_parameters.x),
                                       ('y', self.spectrum_parameters.y),
                                       ('z', self.spectrum_parameters.z),
                                       ('mas', self.spectrum_parameters.mas),
                                       ('ref_kerma', self.spectrum_parameters.ref_kerma),
                                       ('ref_flu', self.spectrum_parameters.ref_flu),
                                       ('brem', self.spectrum_parameters.brem),
                                       ('char', self.spectrum_parameters.char),
                                       ('obli', self.spectrum_parameters.obli)])
    ...
    def flags_dct(self):
        ...
        :return dict flags: An ordered dictionary containing information about the spectrum flags
        """
        flags = ord_dct([('external_spectrum', self.flags.external_spectrum),
                         ('kerma_normalized_spectrum', self.flags.kerma_normalized_spectrum),
                         ('mas_normalized_spectrum', self.flags.mas_normalized_spectrum),
                         ('flu_normalized_spectrum', self.flags.flu_normalized_spectrum)])
    ...
    def save_state_as_json(self, file_name, comment):
        ...
        """
        if not file_name:
            file_name = 'spekpy_state_' + get_current_time_stamp(mode='file') + Const.extension_state_file
        
        if not file_name[-4:] == Const.extension_state_file:
            file_name = file_name + Const.extension_state_file

        
        file_name_full = full_file(Const.dir_data, Const.dir_state_usr, file_name)
...
class FiltrationDef(object):
    """
    A class to hold information on extrinsic filtration (inherent plus added, but not anode self-filtartion)
    """
    def __init__(self):
        self.filters = []
        self.mut_fil = []
